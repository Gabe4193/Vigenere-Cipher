...
def encryption(my_string, my_key):
    ...
    for i in range(0, len(encryption_list)):
        secret_text.append(chr(encryption_list[i]))
    correct_list = ''.join([str(x) for x in secret_text])
    return(correct_list)
...
def decryption(encryption_list, my_key):
    ...
    plaintext_list = []
    for i in range(0, len(decryption_list)):
        plaintext_list.append(chr(decryption_list[i]))
